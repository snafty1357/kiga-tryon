...
import { Shirt, Layers, Shield, Scissors, Star, Footprints, User, Sun, Moon } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      const saved = localStorage.getItem(`kiga_company_slug_${user.email}`);
      if (saved) {
        setCompanySlug(saved);
      } else {
        const prefix = user.email.split('@')[0];
        const sanitized = prefix.toLowerCase().replace(/[^a-z0-9\\-]/g, '').slice(0, 30);
        setCompanySlug(sanitized);
        localStorage.setItem(`kiga_company_slug_${user.email}`, sanitized);
      }
    } else {
      const defaultSaved = localStorage.getItem('kiga_company_slug') || '';
      setCompanySlug(defaultSaved);
    }
  }, [user]);

  const updateCompanySlug = (slug: string) => {
    const sanitized = slug.toLowerCase().replace(/[^a-z0-9\\-]/g, '').slice(0, 30);
    setCompanySlug(sanitized);
    if (user?.email) {
      localStorage.setItem(`kiga_company_slug_${user.email}`, sanitized);
    } else {
      localStorage.setItem('kiga_company_slug', sanitized);
    }
  };
  ...
  const handleGeneratePose = async () => {
    if (!humanFile || !selectedPose) return;
    
    setIsGeneratingPose(true);
    setError(null);
    try {
      const dataUrl = await fileToDataUrl(humanFile);
      const result = await generatePose({
        humanImageUrl: dataUrl,
        pose: selectedPose,
        resolution: resolution,
        format: imageFormat,
      });

      // 取得したURL画像をFileオブジェクトに変換してHumanFileを上書き
      const response = await fetch(result.imageUrl);
      const blob = await response.blob();
      const file = new File([blob], 'posed_model.png', { type: blob.type });
      ...
      setError(err.message || 'ポーズの生成に失敗しました');
    } finally {
  ...
  const analysisToDescription = (analysis: GarmentAnalysis | null): string => {
    ...
    // summaryがあればそれを優先（英語1-2文の要約）
    const parts: string[] = [];
    if (analysis.summary && analysis.summary !== 'Analysis pending') {
      parts.push(analysis.summary);
    }
    if (analysis.type && analysis.type !== 'unknown') parts.push(`Type: ${analysis.type}`);
    if (analysis.color && analysis.color !== 'unknown') parts.push(`Color: ${analysis.color}`);
    if (analysis.pattern && analysis.pattern.toLowerCase() !== 'solid') parts.push(`Pattern: ${analysis.pattern}`);
    if (analysis.buttons && analysis.buttons.toLowerCase() !== 'none' && analysis.buttons !== 'pending') parts.push(`Detail: ${analysis.buttons}`);
    if (analysis.pockets && analysis.pockets.toLowerCase() !== 'none' && analysis.pockets !== 'pending') parts.push(`Feature: ${analysis.pockets}`);
    if (analysis.collar && analysis.collar.toLowerCase() !== 'none' && analysis.collar !== 'pending') parts.push(`Shape: ${analysis.collar}`);
    if (analysis.sleeves && analysis.sleeves.toLowerCase() !== 'none' && analysis.sleeves !== 'pending') parts.push(`Style: ${analysis.sleeves}`);
    if (analysis.material && analysis.material.toLowerCase() !== 'unknown') parts.push(`Material: ${analysis.material}`);
    if (analysis.decorations && analysis.decorations.toLowerCase() !== 'none' && analysis.decorations !== 'pending') parts.push(`Decorations: ${analysis.decorations}`);
  ...
  const handleGenerateQuestions = useCallback(async (): Promise<Question[]> => {
    ...
    try {
      ...
      for (const garment of uploadedGarments) {
        ...
        const questions = await generateQuestions(humanDataUrl, garmentDataUrl, fullDesc || undefined, selectedPose, garment.id);
  ...
  const handleGenerateWithPrompt = useCallback(async (fullPrompt: string) => {
    ...
    try {
      ...
      // 日本語が含まれている場合は、必ず最適化パイプラインを通して英語に翻訳する
      let finalPrompt = fullPrompt;
      if (/[ぁ-んァ-ン一-龥]/.test(fullPrompt)) {
        console.log('[Generate] Auto-optimizing and translating Japanese prompt into English...');
        finalPrompt = await optimizeTryOnPrompt(fullPrompt);
      }

      // プロンプトをポジティブ/ネガティブに分離
      let { positive, negative } = parsePrompt(finalPrompt);
      
      // それでもまだ日本語が混ざっている（極稀なエラー）場合は英語にするように警告を出しておく、もしくはそのまま通す
      ...
      // Supabaseに履歴を保存（モデル・ガーメント画像URLも保存）
      supabase.from('generations').insert({
        ...
        format: imageFormat,
        model_image_url: humanPreview || null,
        garment_image_urls: uploadedGarments
          .filter(g => g.preview)
          .map(g => ({ id: g.id, label: g.label, url: g.preview })),
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            >
              ...
              {/* Gallery Button */}
              <a
                href={companySlug ? `/gallery/${companySlug}` : '/gallery'}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2 text-[11px] px-4 py-2 rounded-lg bg-[#F5F5F5] text-[#78909C] border border-[#E0E0E0] hover:text-[#00BFA5] hover:border-[#00BFA5]/30 hover:bg-[#00BFA5]/5 transition-all duration-300"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {user && (
              <div className="flex items-center gap-4 pl-4 border-l border-[#E0E0E0] md:ml-2">
                {/* 企業ID設定 */}
                <div className="flex items-center gap-1.5" title="共有ギャラリー用ID">
                  <span className="text-[10px] text-[#78909C] font-semibold whitespace-nowrap">企業ID</span>
                  <input
                    type="text"
                    value={companySlug}
                    onChange={(e) => updateCompanySlug(e.target.value)}
                    placeholder="英数字"
                    className="w-20 md:w-28 bg-white border border-[#E0E0E0] rounded px-2 py-1 text-[10px] text-[#333] focus:outline-none focus:border-[#00BFA5] transition-all"
                  />
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-[#E0E0E0]">
                ...
                <div className="mt-3 relative">
                  ...
                  <button
                    ...
                    disabled={isGeneratingPose || !humanFile || !selectedPose}
                    className={`w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-xs font-bold shadow-md transition-all duration-300 mt-3 ${
                      isGeneratingPose || !humanFile || !selectedPose
                        ? 'bg-[#F5F5F5] text-[#444] cursor-not-allowed border border-[#E0E0E0] shadow-none'
                        : 'bg-gradient-to-r from-blue-500 to-indigo-500 text-white hover:shadow-lg hover:scale-[1.02]'
                    }`}
                  >
                    {isGeneratingPose ? (
                      <>
                ...
                {garments.filter(g => g.id === activeGarmentId).map(garment => (
                  ...
                  >
                    <ImageUploader
                      label={garment.label}
                      icon={garment.icon}
                      previewUrl={garment.preview}
                      onFileSelect={handleGarmentSelect(garment.id)}
                      onClear={handleGarmentClear(garment.id)}
